...
namespace
{
	...
	void AppendCString(char* pBuffer, size_t bufferSize, const char* pText)
	{
	...
	void AppendKeyName(char* pBuffer, size_t bufferSize, UINT virtualKey, bool appendSeparator)
	{
		char keyName[32] {};
	...
	void BuildKeyboardKeyString(WORD keyCode, wchar_t* pOutText, size_t outTextSize)
	{
	...
	int DrawWideTextBasic(Surface* pSurface, const wchar_t* pText, const RECT& textRect, BitFont* pFont, COLORREF color)
	{
		if (!pSurface || !pText)
			return 0;
	...
	void EnsureInputCache(OwnerDrawDialogElement& data, const RectangleStruct& screenRect)
	{
	...
	LRESULT PaintInputCtrl(HWND hWnd, OwnerDrawDialogElement& data)
	{
		if (!DSurface::Alternate)
		{
			::ValidateRect(hWnd, nullptr);
	...
	UINT GetStaticAnimationTimerInterval(HWND parentHwnd, HWND controlHwnd)
	{
		int dialogId = 0;
	...
	void DestroyStaticMovie(OwnerDrawDialogElement& data)
	{
	...
	void DestroyStaticMovieAux(OwnerDrawDialogElement& data)
	{
		DeleteUnknownGameObject(data.StaticMovieAuxHandle());
	}

	void DetachStaticMovie(HWND hWnd, OwnerDrawDialogElement& data)
	{
	...
	void PrepareStaticMovieLoad(HWND hWnd, OwnerDrawDialogElement& data)
	{
		DestroyStaticMovie(data);
		::KillTimer(hWnd, 0x65);
		DestroyStaticMovieAux(data);
	...
	LRESULT LoadStaticMovie(HWND hWnd, OwnerDrawDialogElement& data, const char* pMovieName)
	{
	...
	const char* GetStaticMovieName(int index)
	{
	...
	bool EnsureStaticBackground(HWND hWnd, OwnerDrawDialogElement& data)
	{
	...
	void RestoreStaticBackground(HWND hWnd, OwnerDrawDialogElement& data, bool inclusiveBounds)
	{
		if (!data.StaticCachedBackground() || !DSurface::Alternate)
			return;
	...
	void ResetStaticBackground(HWND hWnd, OwnerDrawDialogElement& data)
	{
		if (data.StaticCachedBackground())
	...
	void DrawStaticText(HWND hWnd, OwnerDrawDialogElement& data, const RECT& ownerRect)
	{
	...
	bool DrawStaticPCX(HWND hWnd, OwnerDrawDialogElement& data, RectangleStruct imageRect)
	{
	...
	void DrawStaticShape(HWND hWnd, OwnerDrawDialogElement& data, const RectangleStruct& imageRect, bool animate)
	{
	...
	LRESULT PaintStatic(HWND hWnd, OwnerDrawDialogElement& data)
	{
	...
	void DestroyStaticResources(HWND hWnd, OwnerDrawDialogElement& data)
	{
		if (data.StaticCachedBackground())
			DeleteSurfaceObject(data.StaticCachedBackground());
	...
	LRESULT HandleStaticMovieTimer(HWND hWnd, OwnerDrawDialogElement& data)
	{
		auto pMovie = data.StaticMovieHandle();
		if (!pMovie)
			return 0;
	...
	LRESULT HandleStaticVisualTimer(HWND hWnd, OwnerDrawDialogElement& data)
	{
		const auto drawMode = data.StaticDrawMode();
		if (drawMode == WWUIStaticDrawMode::TypewriterText)
		{
	...
	void DrawTabFrameTiledPCX(const char* pFilename, const RectangleStruct& rect)
	{
		if (auto pPCX = GetPCXSurface(pFilename))
			BlitTiledPCX(rect, DSurface::Alternate, pPCX, 0, 0);
	}

	void DrawTabFrameCornerPCX(const char* pFilename, int x, int y)
	{
	...
	void DrawTabFrame(const RECT& panelRect)
	{
	...
	void DrawTabTransparentPCX(const char* pFilename, const RectangleStruct& rect)
	{
		auto pPCX = GetPCXSurface(pFilename);
		if (!pPCX)
			return;
	...
	void DrawTabItemMiddle(char variant, int drawLeft, int drawTop, int drawWidth)
	{
	...
	void DrawTabItemCaps(char variant, int drawLeft, int drawTop, int drawWidth)
	{
		char filename[32] {};
	...
	void InitializeTabCtrl(HWND hWnd)
	{
		RECT controlRect {};
	...
	LRESULT PaintTabCtrl(HWND hWnd, OwnerDrawDialogElement& data)
	{
		RECT controlRect {};
		OwnerDraw::GetRectangle(hWnd, &controlRect);
	...
	void DrawGroupBoxLine(int startX, int startY, int endX, int endY, int color)
	{
	...
	void DrawGroupBoxPixel(int x, int y, int color)
	{
		Point2D point { x, y };
		DSurface::Alternate->SetPixel(&point, color);
	}

	int MeasureGroupBoxCaption(BitFont* pFont, const wchar_t* pCaption, int maxWidth)
	{
		if (!pFont)
			pFont = BitFont::Instance;
	...
	void DrawGroupBoxCaption(const RECT& groupRect, const wchar_t* pCaption)
	{
	...
	LRESULT PaintGroupBoxCtrl(HWND hWnd, OwnerDrawDialogElement& data)
	{
